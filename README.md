...
should be separated. The data set can be loaded by
...
converts the allele codes to letters, so that each locus now has two letters (there are
also some missing values coded “-”). Finally,

tai <- alleleinit(allelematrix=ta)

produces a collection of ways to represent the data (you don’t need to understand
much of this; if you are curious, consult the help page of alleleinit). Particularly,
tai$distmat is now a matrix of genetic distances (the help page of alleledist
explains how this was computed), which is the data set you are finally asked to
work with.
...
generated by the MDS and apply k-means clustering to them. Again choose a
number of clusters (you may use the gap statistic for this).
